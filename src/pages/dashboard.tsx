...
import { getPerformanceAssessments } from 'apiSdk/performance-assessments';
...
import { PerformanceAssessmentListPage as ShowCaseList } from 'pages/performance-assessments';

function DashboardPage() {
  const { session } = useSession();

  const performanceAssessmentStat = useSWR<PaginatedInterface<PerformanceAssessmentInterface>>(
    () => `/performance-assessments`,
    () => getPerformanceAssessments({ limit: 1, offset: 0 }),
  );

  const vehicleStat = useSWR<PaginatedInterface<VehicleInterface>>(
    () => `/vehicles`,
    () => getVehicles({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: performanceAssessmentStat?.data?.totalCount,
      isLoading: performanceAssessmentStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Vehicles',
      result: vehicleStat?.data?.totalCount,
      isLoading: vehicleStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Reservations',
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Performance Assessments
                </Text>
                <NextLink href={`performance-assessments`} passHref legacyBehavior>
                  <Button
                  ...
                  >
                    See all Performance Assessments
                  </Button>
                </NextLink>
              </Flex>
